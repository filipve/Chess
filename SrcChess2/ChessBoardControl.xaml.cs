    ...
    public partial class ChessBoardControl : UserControl, SearchEngine.ITrace, IXmlSerializable {
        ...
        public ChessBoard.MoveResultE DoMove(ChessBoard.MovePosS movePos, bool bFlash, int iPermCount, int iDepth, int iCacheHit) {
            ...
            if (m_iFlashing == 0) { 
                ...
                eRetVal = SrcChess2.Classes.ChessBoard.MoveResultE.NoRepeat;
            }
            return(eRetVal);
        }
            ...
            if (BoardDesignMode) {
                ...
                if (GetCellFromPoint(e, out pt)) {
                    ...
                    if (SelectedCell.X == -1 || SelectedCell.Y == -1) {
                        ePiece          = m_board[iPos];
                        bWhiteToMove = (m_board.NextMoveColor == SrcChess2.Classes.ChessBoard.PlayerColorE.White);
                        bWhitePiece = (ePiece & SrcChess2.Classes.ChessBoard.PieceE.Black) == 0;
                        if (ePiece != SrcChess2.Classes.ChessBoard.PieceE.None && bWhiteToMove == bWhitePiece)
                        {
                            SelectedCell = pt;
                        } else {
                            System.Console.Beep();
