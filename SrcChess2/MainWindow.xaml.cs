...
namespace SrcChess2 {
    ...
    public partial class MainWindow : Window, ChessBoardControl.IMoveListUI {
        ...
        public virtual void OnCanExecuteCmd(object sender, CanExecuteRoutedEventArgs e) {
            ...
            bIsFlashing = m_chessCtl.IsFlashing;
            if (e.Command == NewGameCommand                     ||
                ...
                e.CanExecute    = !(m_bSecondThreadBusy || bDesignMode || bIsFlashing);
            } else if (e.Command == QuitCommand                 ||
                ...
                e.CanExecute    = !(m_bSecondThreadBusy || bIsFlashing);
            } else if (e.Command == CancelPlayCommand) {
                e.CanExecute    = m_bSecondThreadBusy;
            } else if (e.Command == UndoCommand) {
                e.CanExecute    = (!m_bSecondThreadBusy && !bIsFlashing && !bDesignMode && m_chessCtl.UndoCount >= ((m_ePlayingMode == PlayingModeE.PlayerAgainstPlayer) ? 1 : 2));
            } else if (e.Command == RedoCommand) {
                e.CanExecute    = (!m_bSecondThreadBusy && !bIsFlashing && !bDesignMode && m_chessCtl.RedoCount != 0);
            } else {
                e.Handled   = false;
            }
        }
